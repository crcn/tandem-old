...
class ResizerComponent extends React.Component {
    ...
    startDragging(event) {
        ...
        const sy2 = style.top;
        this._dragger = index_1.startDrag(event, (event2, { delta }) => {
            if (!this.targetPreview.getCapabilities().movable)
                return;
    ...
    render() {
        ...
        return (React.createElement("div", {ref: 'selection', className: 'm-selector-component--selection', style: resizerStyle, onMouseDown: this.startDragging.bind(this), onDoubleClick: this.onDoubleClick.bind(this)}, 
            React.createElement(path_1.default, {showPoints: capabilities.resizable && !this.state.dragging, onPointChange: this.updatePoint.bind(this), zoom: this.props.zoom, points: points, strokeWidth: strokeWidth, pointRadius: pointRadius})
        ));
    }
}
