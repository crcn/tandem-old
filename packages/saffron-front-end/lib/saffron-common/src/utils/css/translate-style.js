...
const css_2 = require('../../tokenizers/css');
...
function translateToScaleProperty(property) {
    if (/left|right|width/.test(property))
        return 'width';
    if (/top|bottom|height/.test(property))
        return 'height';
}
...
function translateLengthToInteger(length, property = undefined, relativeElement = undefined) {
    if (length === '')
        return 0;
    if (isNaN(length) && typeof length === 'number') {
        throw new Error('length cannot be NaN');
    }
    return css_1.default.parse(css_2.default.tokenize(length), astFactory).solveX({
        relativeElement: relativeElement,
        property: property,
    });
}
exports.translateLengthToInteger = translateLengthToInteger;
const astFactory = {
    create(type, props) {
        return this[type] ? this[type](props) : Object.assign({ type: type }, props);
    },
    call(props) {
        return {
            solveX(info) {
...
function tokenize(value) {
    var tokens = css_2.default.tokenize(value);
    return tokens;
}
function translate(fromStyle, toStyle, element) {
    ...
    function translateDeclaration(property2, fromValue, toValue, element2) {
        ...
        // parse into the AST
        const ast = css_1.default.parse(tokens, astFactory);
...
function translateStyleToIntegers(style, relativeElement) {
    let toStyle = {};
    for (const key in style)
        toStyle[key] = '0px';
    toStyle = translate(style, toStyle, relativeElement);
    for (const key in toStyle)
        toStyle[key] = Number(toStyle[key].replace('px', ''));
    return toStyle;
}
exports.translateStyleToIntegers = translateStyleToIntegers;
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = translate;
//# sourceMappingURL=translate-style.js.map