...
export const rootReducer = (state: RootState, action: Action): RootState => {
  ...
  switch(action.type) {
    ...
    case COMPONENT_STATE_NAME_DEFAULT_TOGGLE_CLICK: {
      ...
      const sourceNode = getSyntheticSourceNode(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistComponentStateChanged({ isDefault: value }, name, sourceNode.id, browser), state);
      return state;
    }
    case COMPONENT_STATE_REMOVED: {
      const { name, value } = action as ComponentStateNameDefaultToggleClick;
      const sourceNode = getSyntheticSourceNode(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistRemoveComponentState(name, sourceNode.id, browser), state);
      return state;
    }
    case COMPONENT_STATE_NAME_CHANGED: {
      const { oldName, newName } = action as ComponentStateNameChanged;
      const sourceNode = getSyntheticSourceNode(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistComponentStateChanged({ name: newName }, oldName, sourceNode.id, browser), state);
      return state;
    }
