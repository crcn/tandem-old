...
export const canvasReducer = (state: RootState, action: Action) => {
  switch(action.type) {
    ...
    case SLOT_TOGGLE_CLICK: {
      ...
      state = persistRootStateBrowser(browser => {
        return persistToggleSlotContainer(nodeId, browser);
      }, state);
      return state;
    }
