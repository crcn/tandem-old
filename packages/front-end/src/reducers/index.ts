...
export const rootReducer = (state: RootState, action: Action): RootState => {
  ...
  switch(action.type) {
    ...
    case ELEMENT_VARIANT_TOGGLED: {
      ...
      const sourceNode = getSyntheticSourceNode(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistSetElementVariants(newVariants, sourceNode.id, state.selectedComponentVariantName, browser), state);
      return state;
    }
    case NEW_VARIANT_NAME_ENTERED: {
      const { value } = action as NewVariantNameEntered;
      const sourceNode = getSyntheticSourceNode(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistInsertNewComponentVariant(value, sourceNode.id, browser), state);
      return state;
    }
    case COMPONENT_VARIANT_NAME_DEFAULT_TOGGLE_CLICK: {
      const { name, value } = action as ComponentVariantNameDefaultToggleClick;
      const sourceComponent = getSyntheticNodeSourceComponent(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistComponentVariantChanged({ isDefault: value }, name, sourceComponent.source.id, browser), state);
      return state;
    }
    case COMPONENT_VARIANT_REMOVED: {
      const { name, value } = action as ComponentVariantNameDefaultToggleClick;
      const sourceComponent = getSyntheticNodeSourceComponent(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistRemoveComponentVariant(name, sourceComponent.source.id, browser), state);
      return state;
    }
    ...
    case COMPONENT_VARIANT_NAME_CHANGED: {
      ...
      const sourceComponent = getSyntheticNodeSourceComponent(state.selectedNodeIds[0], state.browser);
      state = persistRootStateBrowser(browser => persistComponentVariantChanged({ name: newName }, oldName, sourceComponent.source.id, browser), state);
      return state;
    }
...
export const canvasReducer = (state: RootState, action: Action) => {
  switch(action.type) {
    ...
    case CANVAS_DROPPED_REGISTERED_COMPONENT: {
      let { item, point, editorUri } = action as CanvasDroppedRegisteredComponent;
      ...
      if (targetNodeId) {
        ...
        point = normalizePoint(getEditorWithFileUri(editorUri, state).canvas.translate, point);
        sourceNode = setNodeAttribute(sourceNode, "style", merge({}, getAttribute(sourceNode, "style"), {
          left: point.left,
          top: point.top,
          width: 200,
          height: 200
      ...
      return persistRootStateBrowser(browser => persistInsertNode(sourceNode, targetSourceId, TreeMoveOffset.APPEND, browser), state);
    }
    ...
    case CANVAS_DRAGGED_OVER: {
      ...
      targetNodeId = getCanvasMouseTargetNodeId(state, action as CanvasToolOverlayMouseMoved, (node) => node.name !== PCSourceTagNames.TEXT);
    ...
    case RAW_CSS_TEXT_CHANGED: {
      ...
      return persistRootStateBrowser(browser => {
        return state.selectedNodeIds.reduce((browserState, nodeId) => persistRawCSSText(cssText, nodeId, state.selectedComponentVariantName, browserState), state.browser)
      }, state);
    }
...
const persistInsertNodeFromPoint = (node: TreeNode, fileUri: string, point: Point, state: RootState) => {
  const targetNodeId = getCanvasMouseTargetNodeIdFromPoint(state, point);
  const oldWindow = getSyntheticWindow(fileUri, state.browser);
  const dep = state.browser.graph[oldWindow.location].content.id;
  const document = targetNodeId && getSyntheticNodeDocument(targetNodeId, state.browser);
  state = persistRootStateBrowser(browser => {
    return persistInsertNode(targetNodeId ? node : setNodeAttribute(node, "style", {
      ...shiftPoint(normalizePoint(getEditorWithFileUri(fileUri, state).canvas.translate, point), {
        left: -(INSERT_ARTBOARD_WIDTH/2),
        top: -(INSERT_ARTBOARD_HEIGHT/2)
      }),
      width: INSERT_ARTBOARD_WIDTH,
      height: INSERT_ARTBOARD_HEIGHT,
    }), targetNodeId ? getSyntheticSourceNode(targetNodeId, browser).id : browser.graph[oldWindow.location].content.id, TreeMoveOffset.APPEND, browser)
  }, state);
...
const shortcutReducer = (state: RootState, action: Action): RootState => {
  switch(action.type) {
    ...
    case SHORTCUT_CONVERT_TO_COMPONENT_KEY_DOWN: {
      ...
      state = persistRootStateBrowser(browser => persistConvertNodeToComponent(state.selectedNodeIds[0], browser), state);
      state = setSelectedSyntheticNodeIds(state);
      return state;
    }
  ...
  switch(action.type) {
    case SYNTHETIC_NODES_PASTED: {
      ...
      if (state.selectedNodeIds.length) {
        ...
        targetSourceNode = state.browser.graph[state.activeEditorFilePath].content;
      }

      const oldWindow = getSyntheticWindow(state.activeEditorFilePath, state.browser);

      state = persistRootStateBrowser(browser => persistPasteSyntheticNodes(state.activeEditorFilePath, targetSourceNode.id, clips, browser), state);
...
const isDroppableNode = (node: TreeNode) => {
  return node.name !== "text" && !/input/.test(String(getAttribute(node, "native-type")));
}
...
const handleArtboardSelectionFromAction = <T extends { sourceEvent: React.MouseEvent<any> }>(state: RootState, nodeId: string, event: T) => {
  ...
  return setSelectedSyntheticNodeIds(state, ...(event.sourceEvent.shiftKey ? [...state.selectedNodeIds, nodeId] : [nodeId]));
}
