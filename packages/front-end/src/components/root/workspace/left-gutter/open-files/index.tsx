...
const BaseOpenFilesPaneComponent = ({ editors, root, dispatch }: OpenFilesPaneInnerProps) => {
  return <PaneComponent header="Open Files" className="m-open-files-pane">
    {
      root.openFiles.map((openFile) => {
        return <OpenFileComponent root={root} key={openFile.uri} openFile={openFile} dispatch={dispatch} active={editors.some(editor => editor.activeFilePath === openFile.uri)}/>
      })
    }
  </PaneComponent>;
};
