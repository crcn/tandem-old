...
export enum SyntheticNodeNames {
  TEXT = "text"
}
...
} & SyntheticBaseTreeNode<string>;

export type SyntheticTextNode = {
  style: KeyValue<any>;
  value: string;
} & SyntheticBaseTreeNode<SyntheticNodeNames.TEXT>;
...
export const isSyntheticDocumentRoot = (node: SyntheticNode) => {
  return node.source.nodeId.length === 1;
};
...
// ): Bounds => {
//   const style = rootSourceNode.style || EMPTY_OBJECT;
...
export const createSyntheticDocument = (
  ...
  const syntheticDocument: SyntheticDocument = {
    root: rootNode,
    container,
    bounds: getPCNodeFrame(rootNode.source.nodeId, module).bounds,
    id: generateUID(),
...
export const findSourceSyntheticNode = (
  ...
  const dep = getPCNodeDependency(node.id, browser.graph);
  ...
  for (const document of window.documents) {
    const synthetic = findNestedNode(
      ...
      (synthetic: SyntheticNode) => {
        return synthetic.source.nodeId === node.id;
      }
    );
    if (synthetic) {
      return synthetic;
...
const updateSyntheticItem = <TTree extends SyntheticBaseTreeNode<any>>(
  ...
  const props: Partial<TreeNode<any>> = properties;
...
export const getSourceNodeById = (
  ...
  const dependency = getPCNodeDependency(nodeId, browser);
  return (
    dependency &&
    (getNestedTreeNodeById(nodeId, dependency.content) as PCNode)
  );
};
...
export const persistPasteSyntheticNodes = (
  ...
  for (const { uri, node, namespaceUris } of clips) {
    ...
    if (sourceNode.name === PCSourceTagNames.COMPONENT) {
      ...
      const child: TreeNode<any> = createTreeNode(
        node.id,
        {
      ...
      const updateNamespaces = (node: TreeNode<any>) => {
        node = {
          ...node,
...
const persistSourceNodeChanges = <TTree extends PCNode>(
  refSourceNode: TTree,
  variantName: string,
  browser: SyntheticBrowser,
  ...
  const sourceDependency = getPCNodeDependency(sourceNode.id, browser);
  ...
  //   const overrideChildren: TreeNode<any>[] = [];
    ...
    browser = updateSyntheticWindow(
      window.location,
      {
        documents: documentNodes.map(newDocumentNode => {
          const sourceComponent = getNestedTreeNodeById(
            newDocumentNode.source.nodeId,
...
const syntheticSourceEquals = (
  ...
  return a.uri === b.uri && a.nodeId === b.nodeId;
};
...
export const persistNewComponent = (
  ...
  const newComponentNode: TreeNode<any> = createComponentNode(dep, bounds);
...
export const persistInsertNode = (
  ...
  const dep = getPCNodeDependency(refSourceNodeId, browser);
...
export const persistConvertNodeToComponent = (
  ...
  const dep = getPCNodeDependency(sourceNode.id, browser);
  ...
  browser = persistSourceNodeChanges(dep.content, null, browser, content => {
    content = replaceNestedNode(
      createTreeNode(componentSourceNode.id),
      sourceNode.id,
      content
    );
...
export const persistMoveSyntheticNode = (
  ...
  const sourceDep = getPCNodeDependency(sourceNode.id, browser);
  const componentInstanceNode = getComponentInstanceSyntheticNode(
    targetNodeId,
...
export const isContainerSyntheticNode = (node: TreeNode<any>) =>
...
export const isCreatedFromComponent = (node: TreeNode<any>) =>
  Boolean(
...
export const getComponentInstanceSourceNode = (
  ...
  const dep = getPCNodeDependency(nodeId, browser);
...
export const persistToggleSlotContainer = (
  ...
  return persistSourceNodeChanges(sourceNode, null, browser, child => {
    const style = child.attributes.core.style || EMPTY_OBJECT;
    const container = child.attributes.core.container;
