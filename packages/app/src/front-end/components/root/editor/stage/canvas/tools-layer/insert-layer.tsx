...
import { Canvas, CanvasToolType } from "front-end/state";
import { CANVAS_MOTION_RESTED, insertToolFinished } from "front-end/actions";
...
const enhance = compose<InsertLayerInnerProps, InsertLayerOuterProps>(
  ...
  withHandlers({
    onMouseDown: ({ setPreviewBounds, canvas, dispatch }: InsertLayerInnerProps) => (startEvent: React.MouseEvent<any>) => {
      ...
      if (canvas.toolType === CanvasToolType.TEXT) {
        return dispatch(insertToolFinished(getBounds()));
      }
      ...
      startDOMDrag(startEvent, () => { }, (event: MouseEvent, { delta }) => {
        ...
        return dispatch(insertToolFinished(getBounds(delta)));
      });
    }
  })
);
