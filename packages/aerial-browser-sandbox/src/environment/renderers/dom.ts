...
export class SyntheticDOMRenderer extends BaseSyntheticWindowRenderer {
  ...
  private tryRegisteringStyleSheet(styleElement: HTMLStyleElement, styleSheet: SEnvCSSStyleSheetInterface) {
    ...
    this._registerCSSRules(nativeStyleSheet, styleSheet);

    return Promise.resolve();
  }

  private _registerCSSRules(native: any, synthetic: any) {
    this._cssRuleDictionary[(synthetic as any).$id] = [native, synthetic];
    if (native.cssRules) {
      for (let i = 0, n = native.cssRules.length; i < n; i++) {
        this._registerCSSRules(native.cssRules[i], synthetic.cssRules[i]);
      }
    } else if (native.style) {
      this._registerCSSRules(native.style, synthetic.style);
    }
  }
  ...
  protected _onWindowMutation(event: SEnvMutationEventInterface) {
    ...
    if (documentMutators[mutation.$type]) {
      ...
      const [nativeCSSObject, syntheticCSSObject] = this.getCSSObjectDictItem(mutation.target);

      if (nativeCSSObject) {
        (cssStyleSheetMutators[mutation.$type] as Mutator<any, any>)(nativeCSSObject, mutation);
      }
    }
  }
