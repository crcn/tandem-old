import { uncompressRootNode, renderDOM, computedDOMInfo, SlimParentNode, patchDOM, pushChildNode, SlimElement, createSlimElement, replaceNestedChild, getVMObjectPath, getVMObjectFromPath, SlimBaseNode, getDocumentChecksum, setVMObjectIds, prepDiff, patchNode2, patchDOM2, renderDOM2, computedDOMInfo2, getVMObjectIdType, SlimVMObjectType, SET_ATTRIBUTE, CSS_SET_STYLE_PROPERTY, SlimCSSStyleRule, getStyleOwnerScopeInfo, getStyleOwnerFromScopeInfo, isCSSPropertyDisabled, VMObject, SlimStyleElement, CSS_DELETE_STYLE_PROPERTY, getStyleValue, CSS_SET_SELECTOR_TEXT, elementMatches, getNestedObjectById, traverseSlimNode, getAttribute, getAttributeValue, INSERT_CHILD_NODE, CSS_INSERT_RULE, getSlimNodeHost, compressRootNode, CSS_DELETE_RULE, SlimCSSGroupingRule } from "slim-dom";
...
function* mutateArtboard(artboard: Artboard, mutations: Mutation<any>[], patchDocument?: boolean) {
  const state: ApplicationState = yield select();
  const workspace = getSelectedWorkspace(state);
...
const findScopedStyle = (scope: string, document: SlimParentNode) => {
  ...
  traverseSlimNode(document, (child) => {
    if (child.type === SlimVMObjectType.ELEMENT && getAttributeValue("scope", child as SlimElement) === scope) {
