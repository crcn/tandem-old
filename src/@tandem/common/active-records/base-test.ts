...
//       });
//       await ar.insert();
//       const chunk = await broker.dispatch(new DSFindRequest(ar.collectionName, ar.serialize() )).read();
//       expect(chunk.value.name).to.equal("a");
//     });
//     it("can update an active record", async () => {
//       await broker.dispatch(new DSInsertRequest("people", { _id: "person1", name: "a", zip: 90210 }));
//       const ar: Person = <Person>ActiveRecordFactoryProvider.find("person", deps).create("people", {
//         _id: "person1",
//         zip: 11111
//       });
//       await ar.update();
//       const chunk = await broker.dispatch(new DSFindRequest(ar.collectionName, { _id: "person1" } )).read();
...
//       const { value } = await broker.dispatch(new DSFindRequest(ar.collectionName, { _id: ar._id })).read();
...
//       await ar.save();
//       ar.sync();
//       await broker.dispatch(new DSUpdateRequest(ar.collectionName, { name: "b" }, { _id: ar._id }));
//       expect(ar.name).to.equal("b");
//       await broker.dispatch(new DSUpdateRequest(ar.collectionName, { name: "c" }, { _id: ar._id }));
...
//       await broker.dispatch(new DSRemoveRequest(ar.collectionName, { _id: ar._id }));
...
//       await ar.save();
//       ar.sync();
//       await broker.dispatch(new DSUpdateRequest(ar.collectionName, { name: "b" }, { _id: ar._id })).readAll();
//       expect(ar.name).to.equal("b");
//       ar.dispose();
//       await broker.dispatch(new DSUpdateRequest(ar.collectionName, { name: "c" }, { _id: ar._id }));
//       expect(ar.name).to.equal("b");
...
//       await broker.dispatch(new DSUpdateRequest(ar.collectionName, { name: "b2" }, { _id: ar2._id }));
//       expect(ar.name).to.equal("b");
