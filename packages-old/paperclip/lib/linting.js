...
var lintComponent = function (component, context) {
    ...
    var usedPreviewNames = {};
    if (!component.id) {
        context = addDiagnosticError(component.source, "id attribute is missing", context);
    }
    for (var i = 0, length_3 = component.source.childNodes.length; i < length_3; i++) {
        var componentChild = component.source.childNodes[i];
        if (componentChild.type === ast_1.PCExpressionType.ELEMENT || componentChild.type === ast_1.PCExpressionType.SELF_CLOSING_ELEMENT) {
            if (ast_1.getElementTagName(componentChild) === "preview") {
                hasPreview = true;
                if (componentChild.type === ast_1.PCExpressionType.SELF_CLOSING_ELEMENT) {
                    context = addDiagnosticWarning(componentChild, "tags must not be self closing", context);
                }
                var name_1 = ast_1.getPCStartTagAttribute(componentChild, "name");
                if (!name_1) {
                    context = addDiagnosticWarning(componentChild, "name attribute is missing", context);
                }
                else {
                    if (usedPreviewNames[name_1]) {
                        context = addDiagnosticError(componentChild, "name already exists", context);
                    }
                    else {
                        var tagChildren = componentChild.childNodes.filter(function (child) { return child.type === ast_1.PCExpressionType.SELF_CLOSING_ELEMENT || child.type === ast_1.PCExpressionType.ELEMENT; });
                        if (tagChildren.length === 0) {
                            context = addDiagnosticError(componentChild, "missing element child", context);
                        }
                    }
                    usedPreviewNames[name_1] = true;
                }
            }
        }
    }
...
var lintNodes = function (nodes, context) {
    for (var i = 0, length_4 = nodes.length; i < length_4; i++) {
...
var lintElement = function (element, context) {
    ...
    if (_repeat) {
        var each = _repeat.each, asValue = _repeat.asValue, asKey_1 = _repeat.asKey;
        var asValueName_1 = asValue.name;
        var asKeyName_1 = asKey_1 && asKey_1.name || undefined;
        context = lintExpr(each, context);
        var currentFilePath_1 = context.currentFilePath;
        var eachValueSource_1 = context.currentExprEvalResult.source;
        utils_1.eachValue(context.currentExprEvalResult.value, function (item, index) {
            context = lintStartTagAttributes(startTag, pushCaller(eachValueSource_1, currentFilePath_1, __assign({}, (context.caller && context.caller.props || {}), (_a = {}, _a[asValueName_1] = item, _a[asKeyName_1] = asKey_1 ? { value: index, source: asKey_1 } : undefined, _a)), context));
...
var lintStartTagAttributes = function (startTag, context) {
    ...
    var attributesByKey = {};
    for (var i = 0, length_6 = startTag.attributes.length; i < length_6; i++) {
    ...
    var callerSource = startTag;
    for (var i = 0, length_7 = startTag.modifiers.length; i < length_7; i++) {
        var modifier = startTag.modifiers[i].value;
...
var addDiagnosticWarning = function (expr, message, context) { return addDiagnostic(expr, parser_utils_1.DiagnosticType.WARNING, message, context); };
...
var lintAttributeValue = function (attribute, context) {
    if (!attribute.value) {
        ...
        for (var i = 0, length_8 = stringBlock.values.length; i < length_8; i++) {
...
var getKeypathOrigin = function (keypath, caller) {
    ...
    var current = prop;
    for (var i = 1, length_9 = keypath.length; i < length_9; i++) {
        var newCurrent = current.value && current.value[i];
        if (!newCurrent)
            break;
        current = newCurrent;
    }
    ...
    switch (expr.type) {
        ...
        case ast_1.BKExpressionType.VAR_REFERENCE: {
            ...
            if (context.caller) {
                ...
                if (value === undefined && (keypath.length > 1 || !context.optionalVars) && context.caller.source.type !== ast_1.BKExpressionType.VAR_REFERENCE && context.caller.source.type !== ast_1.BKExpressionType.PROP_REFERENCE) {
        ...
        case ast_1.BKExpressionType.OBJECT: {
            var object = expr;
            var objValue = {};
            for (var i = 0, length_10 = object.properties.length; i < length_10; i++) {
        ...
        case ast_1.BKExpressionType.ARRAY: {
            var ary = expr;
            var values = [];
            for (var i = 0, length_11 = ary.values.length; i < length_11; i++) {
