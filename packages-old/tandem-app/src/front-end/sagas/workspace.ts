...
function* openDefaultWindow() {
  yield watch((state: ApplicationState) => state.selectedWorkspaceId, function*(
    selectedWorkspaceId,
    state
  ) {
...
function* handleOpenExternalWindowButtonClicked() {
  while (true) {
    const { artboardId }: OpenExternalWindowButtonClicked = yield take(
      OPEN_EXTERNAL_WINDOW_BUTTON_CLICKED
    );
...
function* handleMetaClickElement() {
  while (true) {
    const event: StageToolOverlayClicked = yield take(
      (action: StageToolOverlayClicked) =>
        action.type === STAGE_MOUSE_CLICKED && action.sourceEvent.metaKey
    );
    const state = yield select();
    ...
    try {
      const json = yield call(
        ...
        artboard.checksum,
        getVMObjectPath(node, getDocumentBodyPreview(artboard.document)),
        state
      );
    } catch (e) {
...
function* handleMetaClickComponentCell() {
  while (true) {
    const {
      componentId,
      sourceEvent
    }: ComponentsPaneComponentClicked = yield take(
      (action: StageToolOverlayClicked) =>
        action.type === COMPONENTS_PANE_COMPONENT_CLICKED
    );
  ...
  while (true) {
    ...
    for (const [type, id] of workspace.selectionRefs) {
      ...
      if (
        ...
        workspace.stage.fullScreen.artboardId === id
      ) {
...
function* handleDNDEnded() {
  while (true) {
    ...
    const state: ApplicationState = yield select();
    const workspace = getSelectedWorkspace(state);
    const dropRef = getStageToolMouseNodeTargetReference(
      state,
      event as DNDEvent
    );
...
function* handleDroppedOnEmptySpace(event: DNDEvent) {
  ...
  const workspace = getSelectedWorkspace(state);
  const availableComponent = workspace.availableComponents.find(
  ...
  if (!availableComponent.screenshots.length) {
    return yield put(
      exceptionCaught(new Error(`Component must have a preview tag`))
    );
  }

  const screenshot = availableComponent.screenshots[0];
  const size = screenshot
    ? {
...
function* handleSelectionResized() {
  while (true) {
    let {
      ...
      newBounds,
      sourceEvent
    } = (yield take(RESIZER_PATH_MOUSE_MOVED)) as ResizerPathMoved;
    ...
    for (const item of getBoundedWorkspaceSelection(workspace)) {
      ...
      const scaledBounds = scaleInnerBounds(
        currentBounds,
        currentBounds,
        newBounds
      );
      yield put(
        resized(
          item.$id,
          item.$type,
          scaleInnerBounds(innerBounds, currentBounds, newBounds),
          workspace.targetCSSSelectors
        )
      );
    }
  }
...
function* handleSelectionKeyDown() {
  while (true) {
    ...
    switch (type) {
      case DOWN_KEY_DOWN: {
        yield put(
          resizerMoved(workspaceId, { left: bounds.left, top: bounds.top + 1 })
        );
        break;
      }
      case UP_KEY_DOWN: {
      ...
      case LEFT_KEY_DOWN: {
        yield put(
          resizerMoved(workspaceId, { left: bounds.left - 1, top: bounds.top })
        );
        break;
      }
      case RIGHT_KEY_DOWN: {
...
function* handleSourceClicked() {
  while (true) {
    ...
    yield call(
      ...
      artboard.previewName,
      artboard.checksum,
      getVMObjectPath(item, getDocumentBodyPreview(artboard.document)),
      state
    );
  }
}
...
function* handleSelectionStoppedMoving() {
  while (true) {
    const { point, workspaceId } = (yield take(
      RESIZER_STOPPED_MOVING
    )) as ResizerMoved;
    const state = (yield select()) as ApplicationState;
    const workspace = getWorkspaceById(state, workspaceId);
    for (const item of getBoundedWorkspaceSelection(workspace)) {
      ...
      if (
        workspace.stage.fullScreen &&
        workspace.stage.fullScreen.artboardId === item.$id
      ) {
        continue;
      }
      ...
      yield put(
        stoppedMoving(item.$id, item.$type, workspace.targetCSSSelectors)
      );
    }
  }
}
...
function* shiftSelectedArtboard(indexDelta: number) {
  ...
  const workspace = getSelectedWorkspace(state);
  const artboard =
    getWorkspaceLastSelectionOwnerArtboard(state, state.selectedWorkspaceId) ||
    workspace.artboards[workspace.artboards.length - 1];
  if (!artboard) {
    return;
  }
  ...
  const newIndex =
    change < 0
      ? workspace.artboards.length - 1
      : change >= workspace.artboards.length
        ? 0
        : change;
...
function* handleDeleteComponentsPane() {
  while (true) {
    ...
    const state: ApplicationState = yield select();
    const workspace = getWorkspaceById(state, state.selectedWorkspaceId);
    const componentRefs = workspace.selectionRefs.filter(
      ref => ref[0] === AVAILABLE_COMPONENT
    );

    if (
      componentRefs.length &&
      confirm("Are you sure you want to delete these components?")
    ) {
