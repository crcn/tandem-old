...
var lintAttributeValue = function (attribute, context) {
    if (!attribute.value) {
        ...
        context = lintExpr(attribute.value, context);
        console.log(attribute.value, context.currentExprValue);
    }
    else {
...
var lintExpr = function (expr, context) {
    switch (expr.type) {
        ...
        case ast_1.BKExpressionType.VAR_REFERENCE: {
            ...
            if (context.caller) {
                ...
                if (value == null && (keypath.length > 1 || !context.optionalVars)) {
                    context = addDiagnosticError(context.caller.source, "Missing attribute \"" + keypath.join(".") + "\"", context);
                }
                    ...
                    console.log(inference.type, valueType);
                    if (!(inference.type & valueType)) {
                        context = addDiagnosticError(expr, "Type mismatch: attribute \"" + keypath.join(".") + "\" expecting " + inferencing_1.getPrettyTypeLabelEnd(inference.type) + ", " + inferencing_1.getTypeLabels(valueType) + " provided.", context);
                    }
                }
            }
            return context;
        ...
        case ast_1.BKExpressionType.BIND: {
            var bind = expr;
            return lintExpr(bind.value, __assign({}, context, { optionalVars: false }));
        }
        case ast_1.BKExpressionType.IF:
        case ast_1.BKExpressionType.ELSEIF: {
            var _if = expr;
            return lintExpr(_if.condition, __assign({}, context, { optionalVars: true }));
        }
        default: {
            return context;
        }
...
var setCurrentComponentId = function (componentId, context) { return (__assign({}, context, { componentId: componentId })); };
