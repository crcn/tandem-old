...
var syntheticBrowserReducer2 = function (state, event) {
    switch (event.type) {
        case actions_1.FILE_REMOVED: {
            var publicPath_1 = event.publicPath;
            var browser = aerial_browser_sandbox_1.getSyntheticBrowser(state, state_1.getWorkspaceById(state, state.selectedWorkspaceId).$id);
            var matchingWindow = browser.windows.find(function (window) { return window.location.indexOf(publicPath_1) !== -1; });
            console.log("FILE REMOVED", matchingWindow);
            if (matchingWindow) {
                return aerial_browser_sandbox_1.removeSyntheticWindow(state, matchingWindow.$id);
            }
            return state;
        }
    }
