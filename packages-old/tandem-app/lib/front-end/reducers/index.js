...
var workspaceReducer = function (state, event) {
    switch (event.type) {
        case actions_1.EXCEPTION_CAUGHT: {
            var error = event.error;
            return state_1.updateWorkspace(state, state_1.getSelectedWorkspace(state).$id, {
                uncaughtError: error
            });
        }
        case actions_1.BANNER_CLOSED: {
            return state_1.updateWorkspace(state, state_1.getSelectedWorkspace(state).$id, {
                uncaughtError: null
            });
        }
...
var cssInspectorReducer = function (state, event) {
    switch (event.type) {
        case actions_1.CSS_TOGGLE_DECLARATION_EYE_CLICKED: {
            var _a = event, artboardId = _a.artboardId, itemId = _a.itemId, declarationName = _a.declarationName;
            var workspace = state_1.getArtboardWorkspace(artboardId, state);
            var artboard = state_1.getArtboardById(artboardId, state);
            var disabledStyleDeclarations = workspace.disabledStyleDeclarations || {};
            var scopeInfo = slim_dom_1.getStyleOwnerScopeInfo(itemId, artboard.document);
            var scopeKey = scopeInfo.join("");
            var disabledItemDecls = disabledStyleDeclarations[scopeKey] || {};
            return state_1.updateWorkspace(state, workspace.$id, {
                disabledStyleDeclarations: __assign({}, disabledStyleDeclarations, (_b = {}, _b[scopeKey] = __assign({}, disabledItemDecls, (_c = {}, _c[declarationName] = disabledItemDecls[declarationName] ? null : scopeInfo, _c)), _b))
            });
        }
    }
    return state;
    ...
    switch (event.type) {
        ...
        case actions_1.RESIZER_MOVED: {
            ...
            for (var _i = 0, _d = state_1.getBoundedWorkspaceSelection(workspace); _i < _d.length; _i++) {
                ...
                var newBounds = aerial_common2_1.roundBounds(aerial_common2_1.scaleInnerBounds(itemBounds, selectionBounds, aerial_common2_1.moveBounds(selectionBounds, newPoint)));
        ...
        case actions_1.RESIZER_PATH_MOUSE_MOVED: {
            ...
            for (var _f = 0, _g = state_1.getBoundedWorkspaceSelection(workspace); _f < _g.length; _f++) {
                var item = _g[_f];
                var innerBounds = state_1.getWorkspaceItemBounds(item, workspace);
        ...
        case actions_1.CSS_DECLARATION_TITLE_MOUSE_ENTER: {
            var _h = event, artboardId = _h.artboardId, ruleId = _h.ruleId;
            var artboard = state_1.getArtboardById(artboardId, state);
            // TODO
            return state;
        ...
        case actions_1.BREADCRUMB_ITEM_CLICKED: {
            var _k = event, artboardId = _k.artboardId, nodeId = _k.nodeId;
            var artboard = state_1.getArtboardById(artboardId, state);
        ...
        case actions_1.BREADCRUMB_ITEM_MOUSE_ENTER: {
            ...
            return state_1.updateWorkspace(state, state.selectedWorkspaceId, {
                hoveringRefs: [[slim_dom_1.SlimVMObjectType.ELEMENT, nodeId]]
            });
        }
        ...
        case actions_1.STAGE_MOUNTED:
            {
                ...
                var _o = element.getBoundingClientRect() || {}, _p = _o.width, width = _p === void 0 ? 400 : _p, _q = _o.height, height = _q === void 0 ? 300 : _q;
        ...
        case actions_1.STAGE_MOUSE_MOVED:
            {
                var _r = event.sourceEvent, pageX = _r.pageX, pageY = _r.pageY;
        ...
        case actions_1.STAGE_TOOL_OVERLAY_MOUSE_DOUBLE_CLICKED: {
            var _s = event, sourceEvent = _s.sourceEvent, artboardId = _s.artboardId;
            var workspace = state_1.getArtboardWorkspace(artboardId, state);
            var targetRef = state_1.getStageToolMouseNodeTargetReference(state, event);
            if (!targetRef)
                return state;
        ...
        case actions_1.SELECTOR_DOUBLE_CLICKED: {
            var _t = event, sourceEvent = _t.sourceEvent, item = _t.item;
        ...
        case actions_1.ARTBOARD_SCROLL: {
            var _u = event, artboardId = _u.artboardId, scrollPosition = _u.scrollPosition;
...
var artboardReducer = function (state, event) {
    switch (event.type) {
        case actions_1.ARTBOARD_LOADED: {
            var _a = event, artboardId = _a.artboardId, dependencyUris = _a.dependencyUris, document_1 = _a.document, checksum = _a.checksum, mount = _a.mount;
            return state_1.updateArtboard(state, artboardId, {
        ...
        case actions_1.ARTBOARD_LOADING: {
            var artboardId = event.artboardId;
            state = state_1.updateArtboard(state, artboardId, { loading: true });
            state = state_1.deselectNotFoundItems(state);
            return state;
        }
        case actions_1.FILE_CONTENT_CHANGED: {
            var filePath = event.filePath;
            var workspace = state_1.getSelectedWorkspace(state);
            for (var _i = 0, _b = workspace.artboards; _i < _b.length; _i++) {
                var artboard = _b[_i];
                if (artboard.dependencyUris && artboard.dependencyUris.indexOf(filePath) !== -1) {
                    state = state_1.updateArtboard(state, artboard.$id, { loading: true });
                }
            }
            return state;
        }
        case actions_1.ARTBOARD_PATCHED: {
            var _c = event, artboardId = _c.artboardId, document_2 = _c.document, checksum = _c.checksum, nativeObjectMap = _c.nativeObjectMap;
            var artboard = state_1.getArtboardById(artboardId, state);
            state = state_1.updateArtboard(state, artboardId, {
                document: document_2,
                loading: false,
                nativeObjectMap: nativeObjectMap,
                originalDocument: checksum ? document_2 : artboard.originalDocument,
                checksum: checksum || artboard.checksum
            });
            state = state_1.deselectNotFoundItems(state);
            return state;
        }
        case actions_1.ARTBOARD_DOM_PATCHED: {
            var _d = event, artboardId = _d.artboardId, nativeObjectMap = _d.nativeObjectMap;
            state = state_1.updateArtboard(state, artboardId, {
                loading: false,
                nativeObjectMap: nativeObjectMap
            });
        ...
        case actions_1.ARTBOARD_RENDERED: {
            var _e = event, artboardId = _e.artboardId, nativeObjectMap = _e.nativeObjectMap;
            return state_1.updateArtboard(state, artboardId, {
                nativeObjectMap: nativeObjectMap,
                loading: false
            });
        ...
        case actions_1.STAGE_RESIZED: {
            var _f = event, width = _f.width, height = _f.height;
        ...
        case actions_1.ARTBOARD_DOM_INFO_COMPUTED: {
            var _h = event, artboardId = _h.artboardId, computedInfo = _h.computedInfo;
            return state_1.updateArtboard(state, artboardId, {
        ...
        case actions_1.ARTBOARD_CREATED: {
            ...
            state = state_1.roundArtboardBounds(artboard.$id, state);
            state = state_1.setWorkspaceSelection(state, workspace.$id, aerial_common2_1.getStructReference(artboard));
            return state;
        }
    }
