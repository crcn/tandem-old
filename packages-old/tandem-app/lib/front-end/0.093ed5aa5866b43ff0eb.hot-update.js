...
var inferElement = function (element, context) {
    ...
    var forceOptional = context.forceOptional;
    context = inferStartTag(startTag, context);
    context = inferChildNodes(childNodes, context);
    // optionals are scoped to elements only
    context = __assign({}, context, { forceOptional: forceOptional });
    if (_repeat) {
...
var inferExprType = function (expr, context) {
    switch (expr.type) {
        ...
        case ast_1.BKExpressionType.PROP_REFERENCE: {
            ...
            var forcedOptional = false;
            var keypathStr = keypath.join(".");
            if (context.typeLimit & InferenceType.OPTIONAL) {
                context = setForceOptional(keypathStr, context);
            }
            else if (context.forceOptional[keypathStr]) {
                forcedOptional = true;
...
var setForceOptional = function (name, context) {
    return (__assign({}, context, { forceOptional: __assign({}, context.forceOptional, (_a = {}, _a[name] = true, _a)) }));
    var _a;
};
