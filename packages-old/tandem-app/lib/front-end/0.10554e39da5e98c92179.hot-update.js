...
exports.applicationReducer = function (state, event) {
    ...
    switch (event.type) {
        case actions_1.LOADED_SAVED_STATE: {
            ...
            return lodash_1.merge({}, state, JSON.parse(JSON.stringify(newState)));
        }
...
var stageReducer = function (state, event) {
    switch (event.type) {
        ...
        case actions_1.CSS_DECLARATION_TITLE_MOUSE_ENTER: {
            ...
            var selectorText = aerial_browser_sandbox_1.getSyntheticWindowChild(window_3, ruleId).selectorText;
            return state_1.updateWorkspace(state, state.selectedWorkspaceId, {
                hoveringRefs: aerial_browser_sandbox_1.getMatchingElements(window_3, selectorText).map(function (element) { return [
                    element.$type,
                    element.$id
                ]; })
            });
        ...
        case actions_1.BREADCRUMB_ITEM_CLICKED: {
            ...
            var browser = state_1.getSyntheticWindowBrowser(state, window_4.$id);
            var node = aerial_browser_sandbox_1.getSyntheticNodeById(browser, nodeId);
            var workspace = state_1.getSyntheticWindowWorkspace(state, window_4.$id);
