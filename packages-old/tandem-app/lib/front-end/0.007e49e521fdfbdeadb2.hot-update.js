...
var enhanceComponentsPaneCell = recompose_1.compose(recompose_1.pure, state_1.withDragSource({
    ...
    start: function (props) { return function (event) {
        if (props.screenshots.length) {
            ...
            div.style.backgroundImage = "url(" + screenshot.uri + ")";
            div.style.width = screenshot.clip.right - screenshot.clip.left + "px";
            div.style.height = screenshot.clip.bottom - screenshot.clip.top + "px";
            event.dataTransfer.setDragImage(div, screenshot.clip.left, screenshot.clip.top);
