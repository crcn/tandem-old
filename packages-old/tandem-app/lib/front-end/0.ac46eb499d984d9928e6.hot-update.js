...
var tokenizer_1 = __webpack_require__("../paperclip/lib/tokenizer.js");
...
exports.parseModuleSource = function (source, filePath) {
    ...
    var root = createFragment(context);
    return _memos[source] = {
        root: __assign({}, root, { input: source }),
...
var createElementBlockStatement = function (context) {
    ...
    switch (scanner.curr().value) {
        ...
        case "if": return createConditionBlock(context, ast_1.BKExpressionType.IF);
        case "elseif": return createConditionBlock(context, ast_1.BKExpressionType.ELSEIF);
...
var createArray = function (context) {
    ...
    if (curr.type !== tokenizer_1.PCTokenType.BRACKET_CLOSE) {
        while (1) {
            ...
            if (!exports.testCurrTokenType(context, [tokenizer_1.PCTokenType.COMMA], "Missing , delimiter in array.", ast_utils_1.getLocation(start, scanner.curr(), context.source))) {
                return null;
            }
            scanner.next();
        }
...
var createConditionBlock = function (context, type) {
    var scanner = context.scanner;
    var start = scanner.curr();
    ...
    return ({
        ...
        condition: scanner.curr().type !== tokenizer_1.PCTokenType.BRACKET_CLOSE ? createBKExpressionStatement(context) : null,
        location: ast_utils_1.getLocation(start, scanner.curr(), scanner.source)
    });
};
var createRepeatBlock = function (context) {
    ...
    exports.eatWhitespace(context);
    if (!exports.testCurrTokenType(context, [tokenizer_1.PCTokenType.TEXT], "Repeat block missing collection parameter.", ast_utils_1.getTokenLocation(start, context.source))) {
        return null;
    }
...
var createCSSStyleRuleOrDeclarationProperty = function (context) {
    ...
    while (!scanner.ended()) {
        ...
        if (curr.type === tokenizer_1.PCTokenType.SINGLE_QUOTE || curr.type === tokenizer_1.PCTokenType.DOUBLE_QUOTE) {
            selectorText += curr.value + exports.createString(context).value + curr.value;
        }
        else {
    ...
    if (scanner.curr().type === tokenizer_1.PCTokenType.SEMICOLON || scanner.curr().type === tokenizer_1.PCTokenType.CURLY_BRACKET_CLOSE) {
        ...
        var _a = selectorText.match(/(.+?):(.+)/), match = _a[0], name_1 = _a[1], value = _a[2];
...
var createAttribute = function (context) {
    ...
    var name = getTagName(context);
    var value;
    if (scanner.curr().type === tokenizer_1.PCTokenType.EQUALS) {
        scanner.next(); // eat =
