...
var externalReducer = function (state, event) {
    switch (event.type) {
        case actions_1.OPEN_EXTERNAL_WINDOWS_REQUESTED: {
            console.log("REQ");
            var uris = event.uris;
            ...
            var _loop_1 = function (uri) {
                var window_6 = browser.windows.find(function (window) {
                    return window.location === uri;
                });
                if (!window_6) {
                    window_6 = aerial_browser_sandbox_1.createSyntheticWindow({
                        location: uri,
                        bounds: { left: 0, top: 0, right: 100, bottom: 100 }
                    });
                    state = aerial_browser_sandbox_1.addSyntheticWindow(state, browser.$id, window_6);
                }
                selection.push(window_6);
            };
