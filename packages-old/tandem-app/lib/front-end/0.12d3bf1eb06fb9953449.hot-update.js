    ...
    switch (expr.type) {
        ...
        case ast_1.BKExpressionType.VAR_REFERENCE: {
            ...
            if (context.caller) {
                ...
                context = setCurrentExprEvalResult(value, expr, context);
                if (value === undefined && (keypath.length > 1 || !context.optionalVars)) {
                    context = addDiagnosticError(context.callstack[0].source, "Property \"" + keypath.join(".") + "\" is undefined", context);
                }
                    ...
                    if (!(inference.type & valueType)) {
                        context = addDiagnosticError(context.callstack[0].source, "Type mismatch: attribute \"" + keypath.join(".") + "\" expecting " + inferencing_1.getPrettyTypeLabelEnd(inference.type) + ", " + inferencing_1.getTypeLabels(valueType) + " provided.", context);
                    }
                }
            }
            else {
