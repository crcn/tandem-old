            ...
            if (context.caller) {
                ...
                if (value == null && (keypath.length > 1 || !context.optionalVars)) {
                    ...
                    if (!(inference.type & valueType)) {
                        context = addDiagnosticError(context.caller, "Type mismatch: attribute \"" + keypath.join(".") + "\" expecting " + inferencing_1.getPrettyTypeLabelEnd(inference.type) + ", " + inferencing_1.getTypeLabels(valueType) + " provided.", context);
                    }
                }
            }
            return context;
